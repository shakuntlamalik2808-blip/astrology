...
export const Route = createFileRoute("/admin/whatsapp")({
  head: () => ({
    meta: [
      { title: "WhatsApp Status — Studio" },
      { name: "description", content: "WhatsApp notification connection status." },
      { property: "og:title", content: "WhatsApp Status — Studio" },
      { property: "og:description", content: "WhatsApp notification connection status." },
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="mt-6 text-sm leading-relaxed text-muted-foreground">
        ...
        <pre className="mt-3 overflow-x-auto rounded-sm bg-muted p-4 text-xs text-foreground">firebase functions:secrets:set WHATSAPP_API_TOKEN{"\n"}firebase functions:secrets:set WHATSAPP_PHONE_NUMBER_ID{"\n"}firebase deploy --only functions</pre>
